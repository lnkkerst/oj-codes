...
int main() {
	...
	for(int i = 1; i <= m; ++i) uu[i] = read(), vv[i] = read(), ww[i] = read();
    for(int i = 1; i <= m; ++i) addedge(uu[i], vv[i], ww[i]);
    dij(s);
    for(int i = 1; i <= n; ++i) ans[i] = nodes[i].dis;
    memset(edges, 0, sizeof(edges));
    memset(nodes, 0, sizeof(nodes));
    cnt = 0;
        for(int i = 1; i <= m; ++i) addedge(vv[i], uu[i], ww[i]);
    dij(s);
    int anss = -1;
    for(int i = 1; i <= n; ++i) {
        ans[i] += nodes[i].dis;
        if(ans[i] < (int)1e9) anss = max(anss, ans[i]);
    }
    printf("%d\n", anss);
    return 0;
}