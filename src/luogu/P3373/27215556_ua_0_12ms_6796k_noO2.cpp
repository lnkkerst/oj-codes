...
struct Sg_mtree{
	...
	int query(int a, int b,int l, int r, int x) {
		if(a <= l && b >= r) return sum[x] % mod;
		int mid = (l + r) >> 1, ret = 0;;
		if(mul[x] != 1 || tag[x]) pushdown(l, r, x);
	...
	void mullall(int a, int b, int v, int l, int r, int x) {
		...
		int mid = (l + r) >> 1;
		if(mul[x] != 1 || tag[x]) pushdown(l, r, x);
		if(a <= mid) mullall(a, b, v, l, mid, lc);
		if(b < mid) mullall(a, b, v, mid + 1, r, rc);
		update(x);
	}
} tree;
...
signed main() {
	...
	while(m--) {
		...
		if(opt == 1) tree.mullall(l, r, read(), 1, n, 1);
		if(opt == 2) tree.add(l, r, read(), 1, n, 1);
