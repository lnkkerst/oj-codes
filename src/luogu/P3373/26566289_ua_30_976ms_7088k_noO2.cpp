...
void mul_range(int x, int y, int v) {
  ...
  int tmp = min(y, r[bl[x]]);
  for (int j = x; j <= tmp; ++j) {
    sum[bl[x]] += (v - 1) * a[j], a[j] = a[j] * v;
  }
  if (bl[x] != bl[y]) {
    reset(bl[y]);
    for (int j = y; j >= l[bl[y]]; --j) {
      sum[bl[y]] += (v - 1) * a[j], a[j] = a[j] * v;
    }
  }
  for (int j = bl[x] + 1; j <= bl[y] - 1; ++j) {
    mul[j] *= v, add[j] *= v, sum[j] *= v;
  }
}
...
int query(int x, int y) {
  ...
  for (int j = x; j <= tmp; ++j) {
    ret += a[j] * mul[bl[x]] + add[bl[x]];
  }
  if (bl[x] != bl[y]) {
    for (int j = y; j >= l[bl[y]]; --j) {
      ret += a[j] * mul[bl[y]] + add[bl[y]];
    }
    for (int j = bl[x] + 1; j <= bl[y] - 1; ++j) {
      ret += sum[j];
    }
