...
//     return sqrt(pow(nodes[a].x - nodes[b].x, 2) + pow(nodes[a].y -
//     nodes[b].y, 2));
// }
...
//         for(int i = 1; i <= n; ++i) {
//             if(i == u) continue;
//             if(!nodes[i].pos && calc(i, u) < nodes[i].dis) {
//                 nodes[i].dis = calc(i, u);
//                 heap.insert(make_pair(nodes[i].dis, i));
//             }
//         }
//     }
//     for(int i = 1; i <= n; ++i) {ret += nodes[i].dis;cout << nodes[i].dis;}
//     return ret;
// }
...
double calc(int a, int b) {
  return sqrt(pow(nodes[a].x - nodes[b].x, 2) +
              pow(nodes[a].y - nodes[b].y, 2));
}
