...
int main() {
    ...
    ans[maxa] = maxn;
    for(int i = 1; i <= r; ++i) q[i].a = read(), q[i].b = read();
    sort(q + 1, q + n + 1, cmp);
    ...
    //                 if((ans[j] > ans[na] - 1) || !ans[j]) ans[j] = ans[na] - 1;
    //         }
    //     }
    //     else {
    //         swap(na, nb);
    ...
    //         else {
    //             for(int j = nb + 1; j <= na - 1; ++j)
    //                 if((ans[j] > ans[nb - 1]) || !ans[j]) ans[j] = ans[nb] - 1;
    //         }
    //     }
