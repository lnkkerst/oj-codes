...
bool query(int l, int r) {
	...
	char ch = itl->v;
	for(; itl != itr; ++itl)
		if(itl->v != ch) return 0;
	return 1;
}

bool check(int l, int r) {
	if(l == 1 || l == n || r == 1 || r == n) return query(l, r);
	else {
...
int main() {
	...
	for(int i = 2; i <= n; ++i) {
		ch = readchar();
		if(ch == last) ++cnt;
		else nodes.insert(Node(cntl, cntl + cnt - 1, last)), last = ch, cntl += cnt, cnt = 1;
	}
	nodes.insert(Node(cntl, cntl + cnt - 1, last));
	int m = read();
	for(int i = 1; i <= m; ++i) {
		char opt = readchar(), a = read(), b = read();
		if(opt == 'A') assign(a, b, readchar());
		else puts(check(a, b) ? "Yes" : "No");
	}
	return 0;
}