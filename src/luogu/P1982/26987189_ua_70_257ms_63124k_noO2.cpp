...
//	}
	...
	bool operator >(NumS b) {
		if((NumS){s, isf} == b) return false;
		if(isf && !b.isf) return true;
		if(!isf && b.isf) return false;
		if(!isf && !b.isf) return cmp(b.s, s);
		return cmp(s, b.s);
	}
	bool operator <(NumS b) {
		if((NumS){s, isf} == b) return false;
		if(isf && !b.isf) return false;
		if(!isf && b.isf) return true;
		if(!isf && !b.isf) return cmp(s, b.s);
		return cmp(b.s, s);
	}
	bool operator <=(NumS b) {
		if((NumS){s, isf} == b) return true;
		return (NumS){s, isf} < b;
	}
	bool operator >=(NumS b) {
		if((NumS){s, isf} == b) return true;
    ...
    string div(string a, string b) {
        ...
        while(na > nb) na = na - nb, ret += (NumS){"1", 0};
        return ret.s;
    }
    void operator =(int x) {
		s.clear();
		...
		for(int i = 1; i <= cnt; ++i) s.push_back(nx[cnt - i + 1] + '0');
	}
	NumS operator /(NumS b) {
        NumS ret = (NumS){"", 0}, a = (NumS){s, isf};
        while(a < b) a = a - b, ret = ret + (NumS){"1", 0};
