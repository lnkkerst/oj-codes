...
void dij(int start) {
	...
	while(!heap.empty()) {
		...
		for(int i = nodes[u].h; i; i = edges[i].nex) {
			...
			for(int j = 1; j <= k; ++j) {
				nodes[v].dis[j] = min(nodes[v].dis[j], min(nodes[u].dis[j] + edges[i].w, nodes[u].dis[j - 1]));
			}
			if(!(--nodes[v].d)) heap.push(make_pair(nodes[v].dis[k], v));
		}
	}
}
...
void spfa(int start) {
	...
	while(!q.empty()) {
		...
		for(int i = nodes[u].h; i; i = edges[i].nex) {
			...
			for(int j = 1; j <= k; ++j)
				nodes[v].dis[j] = min(nodes[v].dis[j], min(nodes[u].dis[j] + edges[i].w, nodes[v].dis[j - 1]));
			if(!(--nodes[v].d)) q.push(v);
		}
	}
}
