...
void add(int l, int r, int _v = 1) {
    IT itl = split(l), itr = split(r + 1);
    while(itl != itr) itl->v += _v, ++itl;
}

int calc_rank(int l, int r, int k) {
    vector<pair<int , int > > q; q.clear();
    IT itl = split(l), itr = split(r + 1);
    while(itl != itr) q.push_back(make_pair(itl->v, itl->r - itl->l + 1)), ++itl;
    sort(q.begin(), q.end());
    for(int i = 0; i < (int)q.size(); ++i) {
        k -= q[i].second;
        if(k <= 0) return q[i].first;
    }
    return -1;
}
...
int sum(int l, int r, int ex, int mod) {
    IT itl = split(l), itr = split(r + 1); int ret = 0;
    while(itl != itr) ret = (ret + (itl->r - itl->l + 1) * pow(itl->v, ex, mod)) % mod, ++itl;
    return ret;
}
...
int main() {
    #define int long long
    cin >> n >> m >> seed >> vmax;
    for(int i = 1; i <= n; ++i) a[i] = (rnd() % vmax) + 1, nodes.insert(Node(i, i, a[i]));
    ...
    for(int i = 1; i <= m; ++i) {
        ...
        else if(opt == 3) cout << calc_rank(l, r, x) << endl;
