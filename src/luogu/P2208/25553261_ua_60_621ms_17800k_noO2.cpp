...
void dij(int s) {
	for(int i = 1; i <= n * m << 1; ++i) nodes[i].dis = (int)1e9;
	nodes[s].dis = 0;
...
int main() {
    ...
    for(int i = 1; i <= n; ++i)
        for(int j = 1; j <= m; ++j) {
            char ch = getchar();
            while(ch != '#' && ch != 'C' && ch != '.' && ch != 'D') ch = getchar();
            if(ch == 'C') cx = i, cy = j;
            if(ch == 'D') dx = i, dy = j;
            if(ch == '#') e[toli(i, j)] = 1;
        }
            ...
            if(!e[toli(i, j)]) {
                if(i < n) {
                    if(e[toli(i + 1, j)]) {
                        ...
                        if(j < m && !e[toli(i, j + 1)]) addedge(toli(i, j), toli(i, j + 1), 0);
                        if(j > 1 && !e[toli(i, j - 1)]) addedge(toli(i, j), toli(i, j - 1), 0);
                    }
                    else addedge(toli(i, j), toli(i + 1, j), 0);
                }
                if(i > 1) {
                    if(e[toli(i - 1, j)]) {
                        addedge(toli(i, j) + m * n, toli(i, j), 1);
                        if(j < m && !e[toli(i, j + 1) + m * n]) addedge(toli(i, j) + m * n, toli(i, j + 1) + m * n, 0);
                        if(j > 1 && !e[toli(i, j - 1) + m * n]) addedge(toli(i, j) + m * n, toli(i, j - 1) + m * n, 0);
                    }
                    else addedge(toli(i, j) + m * n, toli(i - 1, j) + m * n, 0);
                }
            }
        }
    }
    dij(toli(cx, cy));
    printf("%d\n", min(nodes[toli(dx, dy)].dis, nodes[toli(dx, dy) + m * n].dis));
    return 0;
}