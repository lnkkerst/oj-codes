...
bool pan(int t) {
  for (int i = t; i >= 1; i--) {
    ...
    if (a[id1] != -1 && a[id2] != -1 && a[id3] != -1) {
      if (mod(a[id1] + a[id2]) != a[id3]
          && mod(a[id1] + a[id2] + 1) != a[id3]) {
        return 0;
      }
    }
    if (i == 1) {
...
void dfs(int t, int tmp) {
  ...
  bool p1, p2, p3;
  for (int i = (a[id1] == -1 ? 0 : a[id1]); i < (a[id1] == -1 ? n : a[id1] + 1);
       i++) {
    if ((a[id1] == -1 && !b[i]) || a[id1] == i) {
      ...
      a[id1] = i;
      b[i] = 1;
      for (int j = (a[id2] == -1 ? 0 : a[id2]);
           j < (a[id2] == -1 ? n : a[id2] + 1); j++) {
        if ((a[id2] == -1 && !b[j]) || a[id2] == j) {
          ...
          a[id2] = j;
          b[j] = 1;
          if ((a[id3] == -1 && !b[mod(a[id1] + a[id2] + tmp)])
              || a[id3] == mod(a[id1] + a[id2] + tmp)) {
            if (!b[mod(a[id1] + a[id2] + tmp)]) {
              p3 = 1;
            } else {
