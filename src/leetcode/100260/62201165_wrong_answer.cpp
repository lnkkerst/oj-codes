...
public:
    ...
    vector<double> intersection(vector<int>& s1, vector<int>& e1, vector<int>& s2, vector<int>& e2) {
		...
		if(d) {
			...
			if(!(t1 >= 0 && t1 <= 1 && t2 >= 0 && t2 <= 1)) return vector<double>{};
			return vector<double>{t1 * s1[0] + (1 - t1) * e1[0], t1 * s1[1] + (1 - t1) * e1[1]};
		}
		if(p || q) return vector<double>{};
		if(s1[0] > e1[0]) swap(s1, e1);
		if(s2[0] > e2[0]) swap(s2, e2);
		if(e1[0] < s2[0] || e2[0] < s1[0]) return vector<double>{};
		if(s1[0] > s2[0]) return vector<double>{(double)s1[0], (double)s1[1]};
		else return vector<double>{(double)s2[0], (double)s2[1]};
	}
};