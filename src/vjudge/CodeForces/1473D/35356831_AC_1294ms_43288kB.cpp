...
int main() {
  ...
  while (t--) {
    ...
    const auto &qMax = [](const vector<vector<int>> &a, const vector<int> &l2,
                          int l, int r) -> int {
      int siz = l2[r - l + 1];
      return max(a[l][siz], a[r - (1 << siz) + 1][siz]);
    };
    const auto &qMin = [](const vector<vector<int>> &a, const vector<int> &l2,
                          int l, int r) -> int {
      int siz = l2[r - l + 1];
    ...
    for (int i = 1; i <= m; ++i) {
      int l, r;
      cin >> l >> r;
      //            int mal = l != 1 ? max(ma[1][l2[l - 1]], ma[l - (1 <<
      //            l2[l - 1])][l2[l - 1]]) : 0; int mar = r != n ?
      //            max(ma[r + 1][l2[n - r]], ma[n - (1 << l2[n - r]) +
      //            1][l2[n - r]]) : 0; int mil = l != 1 ? min(mi[1][l2[l
      //            - 1]], mi[l - (1 << l2[l - 1])][l2[l - 1]]) : 0; int
      //            mir = r != n ? min(mi[r + 1][l2[n - r]], mi[n - (1 <<
      //            l2[n - r]) + 1][l2[n - r]]) : 0;
